...
class TileAreaView: UIView {
    ...
    func createTileArray() {
        let arrayIndexConcatenation = 0
        
        // Image measurements
        let CGIWdith : CGFloat = CGFloat(CGImageGetWidth(self.imageToSolve.CGImage))
        ...
        for index1 in 0..<self.tilesPerRow { // go down the rows
            ...
            for index2 in 0..<self.tilesPerRow { // get the tiles in each row
                ...
                let tileFrame = CGRectMake(totalWidth / 2, totalWidth / 2, 0, 0)
                ...
                let imageFrame = CGRectMake(imagePositionX, imagePositionY, imageWidth, imageWidth)
                let tileCGImage = CGImageCreateWithImageInRect(self.imageToSolve.CGImage, imageFrame)
    ...
    func layoutTiles() {
        ...
        for index1 in 0..<self.tilesPerRow { // go down the rows
            ...
            for index2 in 0..<self.tilesPerRow { // get the tiles in each row
                ...
                let tileFrame = CGRectMake(tileAreaPositionX, tileAreaPositionY, tileWidth, tileHeight)
    ...
    func handleMoveTilePan(gesture:UIPanGestureRecognizer) {
        ...
        if !self.isPuzzleSolved && self.allowTileShifting {
            ...
            case .Began:
                let startingPoint :CGPoint = gesture.locationInView(self)
